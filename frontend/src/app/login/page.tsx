...
export default function LoginPage() {
  // Removed isLoading from useAuth to manage it locally
  const { login } = useAuth();
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  
  // Add local submission state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If login is successful but doesn't automatically redirect, 
      // you might want to add a router.push('/dashboard') here.
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to login');
    } finally {
      // This ensures the spinner STOPS whether the login succeeds or fails
